                ...
                uninstall: false)
    {
        section("Raspberry Pi Server Setup for Valve Control (Optional):") {
            ...
            if (serverEnabled) {
                ...
                href name: "APIPageLink", title: "API Setup", required: false, description: (state.endpoint)?"Setup Complete":"Tap to Enable" , page: "APIPage"
            }
        }
...
def webEvent() {
    ...
    if (data.containsKey("event")) {
        ...
        switch(data.event) {
			...
			def station = data.containsKey('stations')?data.stations.station:1
            def d = getChildDevice(DTHDNI("${data.device_id}:${stations}"))
			log.debug("${data.cycle_volume_gal} cycle reported gallons")
			sendEvent(name:"water_volume_gal", value: cycle_volume_gal, descriptionText:"${cycle_volume_gal} gallons")
...
//log.debug(webEvent(data.status))
def allDeviceStatus() {
    def results = [:]
