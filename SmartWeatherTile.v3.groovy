...
*/
metadata {
    definition (name: "SmartWeather Station Tile v3", namespace: "abuttino", author: "abuttino") {
...
def poll() {
    ...
    if (obs) {
        ...
        sendEvent(name: "power", value: Math.round(obs.dewpoint_f as Double), unit: "F")
		sendEvent(name: "pressure", value: Math.round(obs.pressure_in as Double), unit: "in")
        send(name: "voltage", value: obs.pressure_in)
        ...
        if (dist_units) {
            switch (dist_units) {
            case "dist_mi" :
                send(name: "visibility", value: obs.visibility_mi)
                break;

            case "dist_km":
                send(name: "visibility", value: obs.visibility_km)
                break;
            default:
                send(name: "visibility", value: obs.visibility_mi)
            }
        ...
        if (height_units) {
            switch (height_units) {
            case "height_in" :
                send(name: "percentPrecipToday", value: obs.precip_today_in)
                send(name: "percentPrecipLastHour", value: obs.precip_1hr_in)
                break;

            case "height_mm":
                send(name: "percentPrecipToday", value: obs.precip_today_metric)
                send(name: "percentPrecipLastHour", value: obs.precip_1hr_metric)
                break;
            default:
                send(name: "percentPrecipToday", value: obs.precip_today_in)
                send(name: "percentPrecipLastHour", value: obs.precip_1hr_in)
